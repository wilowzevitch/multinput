...
function Multinput (target, options) {
	...
	/* Create model */
	if ($(this.target+' .item').length>1) {
		this.model = $(this.target+' .item').first().clone(true);
	} else {
		this.model = $(this.target+' .item').clone(true);
	}
	...
	Multinput.prototype.initTransition = function() {
		$(this.target+' .item').css('transition', 'all '+this.options.speed+'ms linear');
	}
	Multinput.prototype.disableTransition = function() {
		$(this.target+' .item').css('transition', 'none');
		$(this.target+' .item').css('transform', 'translateY(0)');
	}
	Multinput.prototype.resetEvents = function() {
		var that = this;
		$(this.target+' [data-action]').off('click');
		$(this.target+' [data-action=remove]').on('click', function() { that.removeItem($(this).parents('.item')); });
		$(this.target+' [data-action=add]').on('click', function() { that.addItem($(this).parents('.item')); });
		$(this.target+' [data-action=moveup]').on('click', function() { that.moveUp($(this).parents('.item')); });
		$(this.target+' [data-action=movedown]').on('click', function() { that.moveDown($(this).parents('.item')); });
	}
	...
	Multinput.prototype.removeItem = function(targetItem) {
		$(this.target+' .item').find('[data-action=remove]').attr('disabled', 'disabled');
		var that = this;
	...
	Multinput.prototype.setButtons = function() {
        $(this.target+' .item [disabled]').removeAttr('disabled');
		$(this.target+' .item').find('[data-action=remove]').attr('disabled', 'disabled');
        $(this.target+' .item').first().find('[data-action=moveup]').attr('disabled', 'disabled');
        $(this.target+' .item').last().find('[data-action=movedown]').attr('disabled', 'disabled');
        if ($(this.target+' .item').length > 1) {
            $(this.target+' .item').find('[data-action=remove]').removeAttr('disabled');
        }
    }
